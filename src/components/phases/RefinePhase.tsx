...
export default function RefinePhase({ track, cycleNumber, completedCycles }: { track: Track; cycleNumber: number; completedCycles: Cycle[] }) {
  ...
  useEffect(() => {
    ...
    }).then(r => r.json()).then(data => { setFeedback(data); setLoading(false); });
  }, []);
  ...
  async function completeCycle() {
    ...
    await supabase.from("cycles").insert({
      track_id: track.id, user_id: user.id,
  ...
  const scoreColor = feedback
    ? feedback.score >= 70 ? "text-green-600" : feedback.score >= 45 ? "text-amber-600" : "text-red-500"
    : "text-stone-400";
  const ringColor = feedback
    ? feedback.score >= 70 ? "#16a34a" : feedback.score >= 45 ? "#b45309" : "#dc2626"
    : "#e5e7eb";

  if (loading) return <div className="py-16 text-center text-sm text-stone-400">Analyzing your work…</div>;

  return (
    <div>
      <div className="flex items-center gap-3 mb-6 pb-4 border-b border-stone-200">
        <div className="flex gap-1.5">
          {["learn","build","refine"].map(p => <div key={p} className={`h-1.5 rounded-full transition-all ${p === "refine" ? "w-5 bg-stone-900" : "w-1.5 bg-stone-200"}`} />)}
        </div>
        ...
        <div className="animate-fade-up space-y-4">
          <div className="text-center py-4">
            <svg width="96" height="96" viewBox="0 0 96 96" className="block mx-auto mb-2">
              ...
              <circle cx="48" cy="48" r="38" fill="none" stroke={ringColor} strokeWidth="5"
            ...
            <p className="text-xs text-stone-400">Cycle {cycleNumber} mastery signal</p>
          </div>

          <div className="bg-stone-100 rounded-2xl p-5">
            <p className="font-serif text-lg text-stone-900 italic leading-snug">"{feedback.headline}"</p>
          </div>
